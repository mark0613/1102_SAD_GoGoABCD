...
<div class="container">
    ...
    <div class="row top">
        ...
        <div class="col-8 center border-2-dark">
            <h3>{{ DB::table("users")->where("u_id", "=", $_GET["c_id"])->first()->username }}</h3>
        </div>
        <div class="col-2"></div>
    </div>
    <div class="row">
        <div class="col-2"></div>
        <div class="col-2 border-2-dark">
            <div class="nav flex-column nav-pills nav-fill" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                @foreach($customers as $cus)
                    ...
                    @if (!empty($_GET["c_id"]) && $_GET["c_id"]==$cus["c_id"])
                        class="nav-link sidebar_font center btn-outline-primary"
                    @else
                        class="nav-link sidebar_font center btn-outline-secondary"
                    @endif
